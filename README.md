...
- 置顶的条目排在所在文件夹列表的最顶端（`../` 之下）：先置顶文件夹、后置顶书籍，各自按置顶顺序排列。
- 置顶标记：
  - 在 CoverBrowser 的**封面网格 / 封面列表**视图中，封面**左上角**绘制一枚图钉角标（黑色图钉 + 白色描边，深色封面上也清晰可见）；
  - 在经典文件名视图中，文件名前显示一个图钉字形。
- 置顶状态保存在 `settings/stick_a_book_on_top.lua`，重启后保留；被删除或移动的条目会自动从置顶中清除。
