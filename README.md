...
- 需要较新版本的 KOReader（带有 `FileManager.addFileDialogButtons` 接口，2024 年中以后的版本均可，已在 v2026.03 实测）。旧版本上插件不会崩溃，但长按选项卡中不会出现置顶按钮。
- 排序和角标绘制逻辑均有 `pcall` 保护：即使未来 KOReader 内部 API 变动，最坏情况只是置顶效果失效并在日志记一条警告，不会影响 KOReader 正常启动和使用。
